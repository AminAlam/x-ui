...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/AminAlam/x-ui/en_language/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
restart() {
    ...
    if [[ $? == 0 ]]; then
        ...
        LOGE "The panel failed to restart, may be the startup time exceeds two seconds, please check the log information later"
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/AminAlam/x-ui/raw/en_language/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_status() {
    ...
    0)
        ...
        ;;
    1)
        echo -e "panel state: ${yellow}has been run${plain}"
        show_enable_status
        ;;
    2)
        echo -e "panel state: ${red}has been run${plain}"
        ;;
    esac
    show_xray_status
}
...
ssl_cert_issue() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            LOGI "The certificate has been issued successfully and is being installed..."
        fi
...
————————————————
  ${green}13.${plain} set up x-ui boot up
