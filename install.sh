...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}mistake：${plain} Note that this script must be run with the root user！\n" && exit 1
...
config_after_install() {
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        read -p "Please type your account name:" config_account
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        last_version=$1
        url="https://github.com/AminAlam/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
